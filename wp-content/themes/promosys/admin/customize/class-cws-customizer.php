...
class CWS_Customizer extends WP_Customize_Control {

	/**
	 * @access public
	 * @var    string
	 */
	public $type = 'cws_customizer';
	...
	public function enqueue() {
		wp_enqueue_script(
			'alpha-color-picker',
			PROMOSYS__URI . 'admin/customize/inc/alpha-color-picker.js',
		...
		wp_enqueue_style(
			'alpha-color-picker',
			PROMOSYS__URI . 'admin/customize/inc/alpha-color-picker.css',
		...
		switch ( $this->type ) {
			case 'alpha-color':
				...
				<div class="control-wrapper alpha-color<?php $this->dependency_state($this->dependency['control'], $this->dependency['operator'], $this->dependency['value']) ?>"<?php echo sprintf('%s', $dep_by); echo sprintf('%s', $dep_operator); echo sprintf('%s', $dep_value); echo sprintf('%s', $dep_def); ?>>
                    ...
                    if ($this->separator == 'line-top') echo '<hr class="top"/>';
			...
			case 'dropdown-pages':
				...
				<div class="control-wrapper dropdown-pages<?php $this->dependency_state($this->dependency['control'], $this->dependency['operator'], $this->dependency['value']) ?>"<?php echo sprintf('%s', $dep_by); echo sprintf('%s', $dep_operator); echo sprintf('%s', $dep_value); echo sprintf('%s', $dep_def); ?>>
					...
					$show_option_none = __( '&mdash; Select &mdash;', 'promosys' );
					$option_none_value = '0';
					...
					<?php if ( $this->allow_addition && current_user_can( 'publish_pages' ) && current_user_can( 'edit_theme_options' ) ) : // Currently tied to menus functionality. ?>
						<button type="button" class="button-link add-new-toggle">
							...
							printf( __( '+ %s', 'promosys' ), get_post_type_object( 'page' )->labels->add_new_item );
							?>
						</button>
						<div class="new-content-item">
							<label for="create-input-<?php echo sprintf('%s', $this->id); ?>"><span class="screen-reader-text"><?php _e( 'New page title', 'promosys' ); ?></span></label>
							<input type="text" id="create-input-<?php echo sprintf('%s', $this->id); ?>" class="create-item-input" placeholder="<?php esc_attr_e( 'New page title&hellip;', 'promosys' ); ?>">
							<button type="button" class="button add-content"><?php _e( 'Add', 'promosys' ); ?></button>
						</div>
					<?php endif; ?>
			...
			case 'link':
				...
				<div class="control-wrapper link">
					...
					<?php if ( !empty( $this->value() ) ) : ?>
						<a target="_blank" href="<?php echo esc_url($this->value()); ?>"><?php echo !empty($this->label) ? esc_html($this->label) : esc_html_x('Link', 'customizer', 'promosys'); ?></a>
					<?php endif; ?>
