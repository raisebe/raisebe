...
function promosys_sanitize_dropdown_pages( $page_id, $setting ) {
 ...
 * @return  mixed
 */ 
function promosys_read_options( $wp_customize, $extensions ){
    $ajax_data = array();
    $count = 0;
                            ...
                            foreach( $section_value as $k => $v ){
                                ...
                                foreach ( $v as $settings_key => $settings_value ){
                                    ...
                                    switch( $settings_key ){
                                        ...
                                        case 'choices':
                                            if( $control_atts['type'] == 'multiple_input' && cws_is_multi_array($settings_value) ){
                                ...
                                if( in_array('wp_image', $control_atts) ){
                                    ...
                                    $wp_customize->add_control( new CWS_Customizer( $wp_customize, $k, $control_atts ) );
                                }
                            }
                        }
                    }
 ...
 * @return  mixed
 */ 
function promosys_get_defaults( $extensions ){
    $defaults = array();
                        ...
                        if( $section_key == 'layout' ){
                            foreach ($section_value as $k => $v) {
                                foreach ($v as $setting_key => $setting_value) {
                                    ...
                                    if( $v['type'] == 'multiple_input' && is_array($setting_value) && cws_is_multi_array($setting_value) ){
 ...
 * @return  mixed
 */ 
function promosys_get_google_fonts(){
    ...
    if( get_option('cws_g_fonts') ){
        $google_fonts = get_option('cws_g_fonts');
    } else {
        $file = PROMOSYS__PATH . 'assets/fonts/gf.json';
 ...
 * @param   array   $def_weight is default value for font-weight control
 ...
 * @return  mixed
 */	
function promosys_typography_control( $location, $def_family, $def_weight, $def_subset, $def_color, $def_size, $def_height ){
    $font_choices = array();
    ...
    $g_fonts = promosys_get_google_fonts();
    ...
    if( !$def_c ){
        ...
        $def_c = array(
            $loc.'_font_color' => array(
                'type'      => 'alpha-color',
                'label'     => esc_html_x(ucfirst($loc).' font Color', 'customizer', 'promosys'),
                'default'   => $def_c,
    ...
    if( $def_s && $def_h ){
        $f_sizes = array(
            $loc.'_font_size' => array(
                'type'      => 'text',
                'default'   => $def_s,
                'label'     => esc_html_x(ucfirst($loc).' font size', 'customizer', 'promosys')
            ),
            $loc.'_font_height' => array(
                'type'      => 'text',
                'default'   => $def_h,
                'label'     => esc_html_x(ucfirst($loc).' line height', 'customizer', 'promosys'),
                'separator' => 'line'
            )
        );
    } else {
    ...
    $out = array_merge(
        array(
            $loc.'_font_family' => array(
                'type'      => 'select',
                'label'     => esc_html_x(ucfirst($loc).' font family', 'customizer', 'promosys'),
                'default'   => $def_f,
                'choices'   => $font_choices
            ),
            $loc.'_font_weight' => array(
                'type'          => 'select',
                'label'         => esc_html_x(ucfirst($loc).' font weight', 'customizer', 'promosys'),
                'default'       => $def_w,
                'choices'       => $font_weight,
                'input_attrs'   => array(
                    'multiple' => true
            ...
            $loc.'_font_subset' => array(
                'type'          => 'select',
                'label'         => esc_html_x(ucfirst($loc).' font subsets', 'customizer', 'promosys'),
                'default'       => $def_ss,
