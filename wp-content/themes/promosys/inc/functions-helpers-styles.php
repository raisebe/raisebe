...
define( 'PRIMARY_COLOR', '#f76331' );
define( 'SECONDARY_COLOR', '#c01fb8' );
define( 'CWS_BACKGROUND_COLOR', '#ffffff' );
const PROMOSYS_FONTS = array('menu', 'titles', 'body');
$GLOBALS['palette'] = array( '#ffffff', '#000000', PRIMARY_COLOR, SECONDARY_COLOR );
if( !isset( $content_width ) ) $content_width = 1170;

if( get_theme_mod('primary_color') ){ 
	if( get_theme_mod('primary_color') != PRIMARY_COLOR ){
		$GLOBALS['palette'][] = esc_attr(get_theme_mod('primary_color'));
	}
}
if( get_theme_mod('secondary_color') ){ 
	if( get_theme_mod('secondary_color') != SECONDARY_COLOR ){
		$GLOBALS['palette'][] = esc_attr(get_theme_mod('secondary_color'));
	}
}
 ...
 * @return  void
 */
function promosys__define_style( $selector, array $attributes ) {
...
function promosys__scheme_styles() {
	...
	$file = PROMOSYS__PATH . 'assets/css/theme_colors.css';
	...
	get_theme_mod('primary_color') ? $primary = esc_attr(get_theme_mod('primary_color')) : $primary = PRIMARY_COLOR;
	get_theme_mod('secondary_color') ? $secondary = esc_attr(get_theme_mod('secondary_color')) : $secondary = SECONDARY_COLOR;
	get_theme_mod('background') ? $background = esc_attr(get_theme_mod('background')) : $background = CWS_BACKGROUND_COLOR;
...
function cws__vc_styles( $styles ) {
    if( !empty( $styles ) ){
    	$styles = trim($styles);
        ...
        wp_add_inline_style( 'cws-footer', $styles );
    }
}
 ...
 * @return  array
 */
function promosys__typography_styles() {
	$fonts_to_load = array();
	...
	wp_enqueue_style( 'promosys-fonts', '//fonts.googleapis.com/css'. $fonts );
}
 ...
 * @return  array
 */
function promosys__background_styles( $options ) {
	$styles = array();
 ...
 * @return  array
 */
function promosys__border_styles( $options ) {
 ...
 * @return  string
 */
function promosys__color_to_rgba( $color, $alpha ) {
