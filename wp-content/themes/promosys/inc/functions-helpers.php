 ...
 * @return  string
 */
function cws_get_metabox($metabox){
    return get_post_meta(get_the_id(), '_cws_'.$metabox, true);
}
 ...
 * @return  bool
 */
function cws_is_multi_array( $arr ) {
    rsort( $arr );
 ...
 * @return  array
 */
function promosys__main_menu_args(){
	return array(
 ...
 * @return  string
 */
function cws_get_taxonomy_links( $tax = "", $delim = "", $layout = 'frontend', $pid = '' ){
 ...
 * @return  string
 */
function cws_get_page_title(){
	$page_title = '';

	if( is_404() ){
		$page_title = esc_html_x('Error Page', 'frontend', 'promosys');
	} else if( is_front_page() ){
		$page_title = esc_html_x('Home', 'frontend', 'promosys');
	} else if( is_search() ){
		$page_title = esc_html_x('Search', 'frontend', 'promosys');
	} else if( is_category() ){
		$cat = get_category( get_query_var( 'cat' ) );
		$cat_name = isset( $cat->name ) ? $cat->name : '';
		$page_title = sprintf( esc_html_x('Category: %s', 'frontend', 'promosys'), $cat_name );
	} else if( is_tag() ){
		$page_title = sprintf( esc_html_x('Tag: %s', 'frontend', 'promosys'), single_tag_title( '', false ) );
	} else if( is_day() ){
	...
	} else if( is_tax( array('cws_portfolio_cat', 'cws_portfolio_tag', 'cws_staff_member_department', 'cws_staff_member_position') ) ){
		...
		$page_title = !empty(get_theme_mod('woo_slug')) ? esc_html(get_theme_mod('woo_slug')) : esc_html_x('Shop', 'frontend', 'promosys');;  
	} else if( get_post_type() == 'cws_staff' ){
		if( is_singular() ){
			$page_title = !empty(get_theme_mod('cws_staff_single_slug')) ? esc_html(get_theme_mod('cws_staff_single_slug')) : esc_html_x('Our Team Single', 'frontend', 'promosys');
		} else {
			$page_title = !empty(get_theme_mod('cws_staff_slug')) ? esc_html(get_theme_mod('cws_staff_slug')) : esc_html_x('Our Team', 'frontend', 'promosys');
		}
	} else if( get_post_type() == 'cws_portfolio' ){
		if( is_singular() ){
			$page_title = !empty(get_theme_mod('cws_portfolio_single_slug')) ? esc_html(get_theme_mod('cws_portfolio_single_slug')) : esc_html_x('Portfolio Single', 'frontend', 'promosys');
		} else {
			$page_title = !empty(get_theme_mod('cws_portfolio_slug')) ? esc_html(get_theme_mod('cws_portfolio_slug')) : esc_html_x('Portfolio', 'frontend', 'promosys');
		}
	} else if( is_archive() ){
		...
		if(is_author()){
			$page_title = sprintf( esc_html_x('Posted by %s', 'frontend', 'promosys'), get_the_author_meta('display_name'));
		}
	} else {
 ...
 * @return  string
 */
function cws_print_template( $template, $type ){
	...
	if( get_theme_mod($template) != 'default' ){
		...
		$cws_template_id = esc_attr(get_theme_mod($template));
		...
		if( !empty($custom_template) ){
			...
			cws__vc_styles($vc_custom_css);

			echo "<div class='cws_".esc_attr($type)."_template'>";
 ...
 * @return  mixed
 */
function promosys__option( $name ) {
	static $options;
	if (empty($options)) {
		global $wp_query;
		$pid = get_the_id();

		$theme_options = get_option(PROMOSYS__ID);
 ...
 * @return  strings
 */
function promosys__current_post_type() {
 ...
 * @since   1.0.0
 */
function promosys_logo( $logo_field = '', $logo_dimensions = '', $tag = 'h1' ){
 ...
 * @since   1.0.0
 */
function promosys__attributes( $attrs ) {
	$attributes = array();
