...
add_filter( 'body_class', 'promosys__body_class' );


function promosys__body_class( $classes ) {
	$classes[] = sprintf( 'sliding-desktop-%s', promosys__option( 'sliding__menuDesktop' ) );
	$classes[] = sprintf( 'sliding-%s', promosys__option( 'sliding__menuStyle' ) );

	return apply_filters( 'promosys__body_class', $classes );
}
 ...
 * @since   1.0.0
 */
function promosys__comments_list() {
 ...
 * @since   1.0.0
 */
function promosys__related_posts() {
	if ( is_single() ) {
 ...
 * @since   1.0.0
 */
function promosys__pagination( $query = null, $max_posts = '', $posts_per_page = '', $type = 'standart', $custom_max_num_pages = '' ) {
	global $wp_query;
	...
	if( !empty($links) ){
		$out .= '<nav class="paging-navigation '.$type.'">';
			$out .= '<div class="pagination loop-pagination">';
				$out .= $type != 'standart' ? '<div class="cws_button cws_load_more">'.esc_html_x( 'Load More', 'frontend', 'promosys' ).'</div>' : $links;
			$out .='</div>';
		$out .= '</nav>';
	}
 ...
 * @since   1.0.0
 */
function promosys_breadcrumbs(){
	/* === OPTIONS === */
	$text['home']	 = esc_html__( 'Home', 'promosys' ); // text for the 'Home' link
	$text['category'] = esc_html__( 'Archive by Category "%s"', 'promosys' ); // text for a category page
	$text['search']   = esc_html__( 'Search for "%s"', 'promosys' ); // text for a search results page
	$text['taxonomy'] = esc_html__( 'Archive by %s "%s"', 'promosys' );
	$text['tag']	  = esc_html__( 'Posts Tagged "%s"', 'promosys' ); // text for a tag page
	$text['author'] = esc_html__( 'Articles Posted by %s', 'promosys' ); // text for an author page
	$text['404']	  = esc_html__( '404 Page', 'promosys' ); // text for the 404 page

	$show_current   = 1; // 1 - show current post/page/category title in breadcrumbs, 0 - don't show
	$show_on_home   = 0; // 1 - show breadcrumbs on the homepage, 0 - don't show
	$show_on_404   	= 1; // 1 - show breadcrumbs on the 404, 0 - don't show
	...
	if ( is_home() || is_front_page() ) {
		...
		if ( is_category() ) {
		...
		} else if ( is_tax( array( 'cws_portfolio_tag', 'cws_portfolio_cat', 'cws_staff_member_department', 'cws_staff_member_position' ) ) ) {
			...
			if ( $show_current ) {
                ...
                if( $post_type == 'cws_portfolio' || $post_type == 'cws_staff' ){
		...
		if ( get_query_var( 'paged' ) ) {
			echo sprintf("%s", $delimiter) . esc_html__( 'Page', 'promosys' ) . ' ' . get_query_var( 'paged' );
		}
 ...
 * @since   1.0.0
 */
function promosys__get_sidebar( $id = null, $custom_sidebar = '', $custom_sidebar_pos = '', $extra_class = '',
                            $show_empty = false, $show_socials = false ){
	...
	if( !empty($check_sidebar) ){
		$out .= '<aside class="sidebar position_'.esc_attr($custom_sidebar_pos).' '.(!empty($extra_class) ? $extra_class : '').'">';
            ...
            $out .= !empty($social_share) && $show_socials ? $social_share : '';
		$out .= '</aside>';
	} else if( empty($check_sidebar) && !empty($custom_sidebar) && $show_empty == true ){
		$out = '<aside class="sidebar position_'.esc_attr($custom_sidebar_pos).' '.(!empty($extra_class) ? $extra_class : '').'">';
			$out .= '<p class="empty_custom_sb_title">'.esc_html_x('Fill the custom sidebar or choose anouther one.', 'frontend', 'promosys').'</p>';
		$out .= '</aside>';
	}
