...
if( !empty(cws_get_metabox('product_slides_count')) ){
	$thumbnails_quantity = cws_get_metabox('product_slides_count');
...
$data .= ' data-show='.esc_attr($thumbnails_quantity).'';
...
<div class="<?php echo esc_attr( implode( ' ', array_map( 'sanitize_html_class', $wrapper_classes ) ) ); ?>" style="opacity: 0; transition: opacity .25s ease-in-out;" <?php echo esc_attr($data); ?>>
	<figure class="woocommerce-product-gallery__wrapper">
        <?php do_action( 'woocommerce_product_before_gallery' );
		if ( $product->get_image_id() ) {
			$html = wc_get_gallery_image_html( $post_thumbnail_id, true );
		} else {
			$html  = '<div class="woocommerce-product-gallery__image--placeholder">';
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src( 'woocommerce_single' ) ), esc_html__( 'Awaiting product image', 'promosys' ) );
			$html .= '</div>';
		}
