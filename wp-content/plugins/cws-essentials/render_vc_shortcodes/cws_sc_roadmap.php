...
function cws_vc_shortcode_sc_roadmap ( $atts = array(), $content = "" ){
	$defaults = array(
		...
		"custom_styles"				=> "",
		"shape_bg"					=> "#eeeeee",
		"active_shape_bg"			=> SECONDARY_COLOR,
		"icon_color"				=> '',
		"active_icon_color"			=> '#ffffff',
		"number_color"				=> "",
		"active_number_color"		=> "#ffffff",
		"number_background"			=> "#dddddd",
		"active_number_background"	=> PRIMARY_COLOR,
		"title_color"				=> "",
		"text_color"				=> "",
	...
	$responsive_vars = array(
		"all" => array(
			...
			"title_size"		=> "20px",
			"title_margins"		=> "25px 0px 0px 0px",
			"text_size"			=> "16px",
		),
	);
	...
	if( $customize_size ){
		if( !empty($title_size) ){
			$styles .= "
				#".$id." .cws_roadmap_item .roadmap_content_wrapper .roadmap_title{
	...
	if( !empty($shape_bg) ){
		$styles .= "
			#".$id." .cws_roadmap_item .roadmap_icon_wrapper{
	...
	if( !empty($active_shape_bg) ){
		$styles .= "
			#".$id." .cws_roadmap_item.active .roadmap_icon_wrapper{
	...
	if( !empty($icon_color) ){
		$styles .= "
			#".$id." .cws_roadmap_item .roadmap_icon_wrapper > i{
	...
	if( !empty($active_icon_color) ){
		$styles .= "
			#".$id." .cws_roadmap_item.active .roadmap_icon_wrapper > i{
	...
	if( !empty($number_color) ){
		$styles .= "
			#".$id." .cws_roadmap_item .roadmap_icon_wrapper .number{
	...
	if( !empty($active_number_color) ){
		$styles .= "
			#".$id." .cws_roadmap_item.active .roadmap_icon_wrapper .number{
	...
	if( !empty($text_color) ){
		$styles .= "
			#".$id." .cws_roadmap_item .roadmap_content_wrapper .roadmap_description{
	...
	if( !empty($line_color) ){
		$styles .= "
			#".$id." .cws_roadmap_item .roadmap_divider{
	...
	if( !empty($arrow_color) ){
		$styles .= "
			#".$id." .cws_roadmap_item .roadmap_divider:after{
	...
	$out .= "<div id='".$id."' class='cws_roadmap_module ".$extra_classes." ".esc_attr($el_class)."'>";
		...
		foreach ($values as $value) {
			...
			/* -----> Roadmap Item <----- */
			$out .= "<div class='cws_roadmap_item".( isset($value['active']) ? ' active' : '' )."'>";
