...
function cws_vc_shortcode_sc_search ( $atts = array(), $content = "" ){
	$defaults = array(
		/* -----> GENERAL TAB <----- */
		"label"						=> "Search",
		"el_class"					=> "",
	...
	$out .= "<div id='".$id."' class='cws_search_module". ( !empty($el_class) ? " $el_class" : "" ) ."'>";
		$out .= "<div class='search-trigger'>";
			$out .= !empty($label) ? "<span class='search-trigger-text'>" . esc_html($label) . "</span>" : "";
		$out .= "</div>";
	$out .= "</div>";

	return $out;
}
add_shortcode( 'cws_sc_search', 'cws_vc_shortcode_sc_search' );
