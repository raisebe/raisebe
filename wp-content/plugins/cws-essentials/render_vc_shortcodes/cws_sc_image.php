...
function cws_vc_shortcode_sc_image ( $atts = array(), $content = "" ){
	...
	$id = uniqid( "cws_image_" );
	$image_alt = get_post_meta($image, '_wp_attachment_image_alt', TRUE);
//	$image = !empty($image) ? wp_get_attachment_image_src($image, 'full')[0] : '';
    $image_src = !empty($image) ? wp_get_attachment_image_url( $image, $thumbnail_size ) : '';
    $image_srcset = !empty($image) ? wp_get_attachment_image_srcset( $image, $thumbnail_size ) : '';
    $image_sizes = !empty($image) ? wp_get_attachment_image_sizes($image, $thumbnail_size) : '';
...
//	$hover_image = !empty($hover_image) ? wp_get_attachment_image_src($hover_image, 'full')[0] : '';
    $hover_image_src = !empty($hover_image) ? wp_get_attachment_image_url( $hover_image, $thumbnail_size ) : '';
    $hover_image_srcset = !empty($hover_image) ? wp_get_attachment_image_srcset( $hover_image, $thumbnail_size ) : '';
    $hover_image_sizes = !empty($hover_image) ? wp_get_attachment_image_sizes($hover_image, $thumbnail_size) : '';
	...
	if( !empty($image) ){
		...
		$out .= "<div id='".$id."' class='cws_image_module".$module_classes."' ".esc_attr($module_atts).">";

			$out .= '<div class="main_image">';
				if( !empty($hover_image) ){
					$out .= '<img class="hover_image" src="'.$hover_image_src.'" srcset="'.esc_attr($hover_image_srcset).'" sizes="'.esc_attr($hover_image_sizes).'" alt="'.esc_attr($hover_image_alt).'" />';
				}
				$out .= '<img class="image" src="'.$image_src.'" srcset="'.esc_attr($image_srcset).'" sizes="'.esc_attr($image_sizes).'" alt="'.esc_attr($image_alt).'" />';
			$out .= '</div>';
