...
function cws_vc_shortcode_carousel ( $atts, $content ){
	$defaults = array(
		...
		'custom_colors'				=> true,
		'nav_color'					=> '#ffffff',
		'nav_hover_color'			=> SECONDARY_COLOR,
		'dots_color'				=> '#c9c9c9',
		'dots_active_color'			=> SECONDARY_COLOR,
	);
	...
	if( $custom_colors ){
		...
		if( !empty($nav_hover_color) ){
			$styles .= "
				...
				{
					#".$id." .cws_carousel .slick-arrow:hover{
	...
	if ( !empty( $content ) ){
		$out .= "<div id='".$id."' class='cws_carousel_wrapper ".esc_attr($el_class)."'". ( !empty($section_atts) ? $section_atts : "" ) .">";

			$shortcode = do_shortcode($content);
			$count = 1;

			if( preg_match_all('/woocommerce/', $shortcode) != 0 ){
				$shortcode = preg_replace('/products/', 'products cws_carousel', $shortcode, $count);
				$out .= $shortcode;
			} else {
				$out .= "<div class='cws_carousel cws_wrapper'>";
					$out .= $shortcode;
				$out .= "</div>";
			}
