...
function cws_vc_shortcode_sc_gallery ( $atts = array(), $content = "" ){
	$defaults = array(
		/* -----> GENERAL TAB <----- */
		'columns'					=> '3',
		'values'					=> '',
	...
	if( !empty($overlay_gradient_1) && !empty($overlay_gradient_2) ){
		$styles .= "
			.".$id." .cws_gallery_image .cws_gallery_overlay{
	...
	$module_classes = $id;
	$module_classes .= ' cws_gallery_wrapper';
	$module_classes .= !empty($el_class) ? " $el_class" : "";

	$end_tag = '</a>';

	$out .= "<div class='".$module_classes."'>";
		$out .= "<div class='cws_gallery_images columns_".$columns . ($enable_masonry ? ' masonry' : '') . ($url == 'media' ? ' magnific' : '')."'>";
			...
			foreach( $values as $value ){
				...
				$out .= "<".$start_tag." class='cws_gallery_image'>";
					$out .= "<img src='".$image."' alt='".esc_attr($image_alt)."' />";
					$out .= "<div class='cws_gallery_overlay'>";
...
add_shortcode( 'cws_sc_gallery', 'cws_vc_shortcode_sc_gallery' );
