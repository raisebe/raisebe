...
function cws_vc_shortcode_sc_breadcrumbs ( $atts = array(), $content = "" ){
	$defaults = array(
		/* -----> GENERAL TAB <----- */
		"label"					=> "Search",
		"el_class"				=> "",
	...
	if( !is_front_page() ){
		$out .= "<div id='".$id."' class='cws_breadcrumbs_module". ( !empty($el_class) ? " $el_class" : "" ) ."'>";
			ob_start();
				promosys_breadcrumbs();
			$breadcrumbs = ob_get_clean();

			if( !empty($breadcrumbs) ){
				$out .= sprintf('%s', $breadcrumbs);
			}
		$out .= "</div>";
	}
...
add_shortcode( 'cws_sc_breadcrumbs', 'cws_vc_shortcode_sc_breadcrumbs' );
